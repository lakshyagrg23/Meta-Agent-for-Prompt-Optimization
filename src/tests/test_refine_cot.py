"""
Tests for src/mutations/refine_cot.py

Covers:
- successful CoT mutation
- isolated component mutation
- original state immutability
- concise bounded refinement behavior (token budget preservation)
...
from src.mutations.refine_cot import RefineCoTOperator
...
def get_context() -> MutationContext:
    signals = CriticSignals(
        ...
        high_fp=False,
        low_accuracy=False,
        inconsistent=True,  # Simulate one active signal typical for CoT refinement
        plateau=False
    )
    return MutationContext(signals=signals)

def test_successful_cot_mutation():
    state = get_valid_state()
    op = RefineCoTOperator()
    
    result = op.mutate(state, get_context())
    
    assert result.success is True
    # The deterministic placeholder outputs: "Refined cot fixing 1 signals."
    assert "Refined cot" in result.candidate_state.cot.content
    assert result.candidate_state.cot.revision_count == 1
    assert "RefineCoTOperator" in result.operator_name
    print("test_successful_cot_mutation PASSED")
...
def test_token_budget_preservation():
    state = get_valid_state()
    budget_before = state.cot.token_budget
    
    op = RefineCoTOperator()
    result = op.mutate(state, get_context())
    
    # Check that structural integer is untouched
    assert result.candidate_state.cot.token_budget == budget_before
...
def test_original_state_immutability():
    ...
    assert state.cot.content == original_state.cot.content
    assert state.cot.revision_count == original_state.cot.revision_count
...
def test_deterministic_placeholder_compatibility():
    ...
    assert result1.candidate_state.cot.content == result2.candidate_state.cot.content
